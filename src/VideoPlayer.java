...
public class VideoPlayer extends CordovaPlugin implements OnCompletionListener, OnPreparedListener, OnErrorListener, OnDismissListener {
    ...
    protected void openVideoDialog(String path, JSONObject options) {
		...
		Resources resources = cordova.getActivity().getApplicationContext().getResources();
		int resourceId = resources.getIdentifier("back_button_image", "drawable",cordova.getActivity().getApplicationContext().getPackageName());
		...
		rparams.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM,1);
		rparams.addRule(RelativeLayout.CENTER_HORIZONTAL,1);
		main.addView(imageView,rparams);
